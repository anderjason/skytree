...
exports.colorsGivenCanvas = void 0;
const Color_1 = require("../../Color");
const ArrayUtil_1 = require("../../ArrayUtil");
function colorsGivenCanvas(inputCanvas, maxColors) {
    ...
    results = ArrayUtil_1.ArrayUtil.arrayWithOrderFromValue(results, (r) => r.frequency, true);
    const clusters = [];
    let needsCluster = [...results];
    const distanceThreshold = 10;
    while (needsCluster.length > 1) {
        const targetColor = Color_1.Color.givenHex(needsCluster[0].hexColor);
        const removeHexColors = new Set();
        const cluster = needsCluster.filter((item) => {
            const itemColor = Color_1.Color.givenHex(item.hexColor);
    ...
    let colors = results.map((r) => Color_1.Color.givenHex(r.hexColor));
    const firstColor = colors[0];
    colors = ArrayUtil_1.ArrayUtil.arrayWithOrderFromValue(colors, (color) => {
        return firstColor.toDistanceGivenColor(color);
    });
    return colors;
}
exports.colorsGivenCanvas = colorsGivenCanvas;
//# sourceMappingURL=colorsGivenCanvas.js.map