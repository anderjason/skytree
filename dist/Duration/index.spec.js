...
describe("Duration", () => {
    it("can be created from milliseconds", () => {
        const duration = _1.Duration.givenMilliseconds(100);
        assert(duration.toMilliseconds() === 100);
    });
    it("can be created from seconds", () => {
        const duration = _1.Duration.givenSeconds(10);
        assert(duration.toMilliseconds() === 10000);
    });
    it("can be created from minutes", () => {
        const duration = _1.Duration.givenMinutes(1);
        assert(duration.toSeconds() === 60);
    });
    it("can be created from hours", () => {
        const duration = _1.Duration.givenHours(1);
        assert(duration.toMinutes() === 60);
    });
    it("can be created from days", () => {
        const duration = _1.Duration.givenDays(1);
        assert(duration.toHours() === 24);
    });
    it("can be created from the time between two instants", () => {
        const start = Instant_1.Instant.givenEpochMilliseconds(1586569320000);
        const end = Instant_1.Instant.givenEpochMilliseconds(1586569500000);
        const duration = _1.Duration.givenInstantRange(start, end);
        assert(duration.toMinutes() === 3);
    });
    it("can be converted to milliseconds", () => {
        const duration = _1.Duration.givenSeconds(0.5);
        assert(duration.toMilliseconds() === 500);
    });
    it("can be converted to seconds", () => {
        const duration = _1.Duration.givenMilliseconds(500);
        assert(duration.toSeconds() === 0.5);
    });
    it("can be converted to minutes", () => {
        const duration = _1.Duration.givenSeconds(30);
        assert(duration.toMinutes() === 0.5);
    });
    it("can be converted to hours", () => {
        const duration = _1.Duration.givenMinutes(30);
    ...
    it("can create a promise of a delay", () => __awaiter(void 0, void 0, void 0, function* () {
        ...
        const delayMs = 50;
        yield _1.Duration.givenMilliseconds(delayMs).toDelay();
        const end = Instant_1.Instant.ofNow();
        const elapsed = _1.Duration.givenInstantRange(start, end);
