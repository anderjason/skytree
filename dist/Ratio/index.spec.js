...
describe("Ratio", () => {
    it("can be created from a decimal", () => {
        const ratio = _1.Ratio.givenDecimal(0.5);
        assert(ratio.toString() === "0.5");
    });
    it("can be created from a fraction", () => {
        const ratio = _1.Ratio.givenFraction(2, 10);
        assert(ratio.toString() === "0.2");
    });
    it("can be created from a value and range", () => {
        const ratio = _1.Ratio.givenValueAndRange(20, 0, 100);
        assert(ratio.toString() === "0.2");
    });
    it("can be created from a percent", () => {
        const percent = Percent_1.Percent.givenNumber(50);
        const ratio = _1.Ratio.givenPercent(percent);
        assert(ratio.toString() === "0.5");
    });
    ...
    it("can be added to another ratio", () => {
        const ratio1 = _1.Ratio.givenDecimal(0.5);
        const ratio2 = _1.Ratio.givenDecimal(0.2);
    ...
    it("can be multiplied by another ratio", () => {
        const ratio1 = _1.Ratio.givenDecimal(0.5);
        const ratio2 = _1.Ratio.givenDecimal(0.5);
