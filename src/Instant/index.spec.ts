...
describe("Instant", () => {
  ...
  it("can be created from an epoch milliseconds value", () => {
    const instant = Instant.ofEpochMilliseconds(1586569500000);
    assert(instant.toEpochMilliseconds() === 1586569500000);
  });

  it("can be converted to a native date", () => {
    const instant = Instant.ofEpochMilliseconds(1586484305000);
  ...
  it("can add a duration", () => {
    const instant = Instant.ofEpochMilliseconds(1586484305000);
    const result = instant.withAddedDuration(Duration.ofMinutes(3));
